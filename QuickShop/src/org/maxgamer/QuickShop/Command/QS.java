    ...
    private void export(CommandSender sender, String[] args)
    {
        ...
        if (type.startsWith("mysql"))
        {
            ...
            try
            {
                ...
                sender.sendMessage(
                    ChatColor.GREEN + "Success - Exported to MySQL " + user + "@" + host + "." +
                    name);
            }
            catch (Exception e)
            {
                e.printStackTrace();
                sender.sendMessage(
                    ChatColor.RED + "Failed to export to MySQL " + user + "@" + host + "." + name +
        ...
        if (type.startsWith("sql") || type.contains("file"))
        {
            ...
            try
            {
                ...
                QuickShop.instance.getDB().copyTo(target);
                sender.sendMessage(
                    ChatColor.GREEN + "Success - Exported to SQLite: " + file.toString());
            }
            catch (Exception e)
            {
                e.printStackTrace();
                sender.sendMessage(
                    ChatColor.RED + "Failed to export to SQLite: " + file.toString() +
                    " Reason: " + e.getMessage());
            }
            return;
        }
        ...
        if (sender instanceof Player && sender.hasPermission("quickshop.create.buy"))
        {
            ...
            while (bIt.hasNext())
            {
                ...
                if (shop != null && shop.getOwner().equalsIgnoreCase(((Player)sender).getName()))
                {
                    ...
                    sender.sendMessage(
                        MsgUtil.getMessage("command.now-buying", shop.getDataName()));
                    return;
                }
            }
            sender.sendMessage(MsgUtil.getMessage("not-looking-at-shop"));
        ...
        if (sender instanceof Player && sender.hasPermission("quickshop.create.sell"))
        {
            ...
            while (bIt.hasNext())
            {
                ...
                if (shop != null && shop.getOwner().equalsIgnoreCase(((Player)sender).getName()))
                {
                    ...
                    shop.update();
                    sender.sendMessage(
                        MsgUtil.getMessage("command.now-selling", shop.getDataName()));
                    return;
                }
            }
            sender.sendMessage(MsgUtil.getMessage("not-looking-at-shop"));
    ...
    private void setPrice(CommandSender sender, String[] args)
    {
        if (sender instanceof Player && sender.hasPermission("quickshop.create.changeprice"))
        {
            ...
            while (bIt.hasNext())
            {
                ...
                if (shop != null &&
                    (shop.getOwner().equalsIgnoreCase(((Player)sender).getName()) ||
                     sender.hasPermission("quickshop.other.price")))
                {
                    ...
                    if (fee > 0)
                    {
                        ...
                        {
                            sender.sendMessage(
                                MsgUtil.getMessage("you-cant-afford-to-change-price",
                                    plugin.getEcon().format(fee)));
                            return;
                        }
                    ...
                    shop.setSignText();
                    shop.update();
                    sender.sendMessage(MsgUtil.getMessage("price-is-now",
                        plugin.getEcon().format(shop.getPrice())));
                    ...
                    if (shop instanceof ContainerShop)
                    {
                        ...
                        if (cs.isDoubleShop())
                        {
                            ...
                            if (cs.isSelling())
                            {
                                ...
                                {
                                    sender.sendMessage(
                                        MsgUtil.getMessage("buying-more-than-selling"));
                                }
                            }
                            else
                            {
        ...
        if (args.length > 0)
        {
            ...
            else if (subArg.equals("info"))
            {
                if (sender.hasPermission("quickshop.info"))
                {
                    ...
                    for (HashMap<ShopChunk, HashMap<Location, Shop>> inWorld : plugin.getShopManager().getShops().values())
                    {
                        ...
                        for (HashMap<Location, Shop> inChunk : inWorld.values())
                        {
                            ...
                            for (Shop shop : inChunk.values())
                            {
                                ...
                                if (shop instanceof ContainerShop &&
                                    ((ContainerShop)shop).isDoubleShop())
                                {
                                    doubles++;
                                }
                    ...
                    sender.sendMessage(
                        ChatColor.GREEN + "" + (buying + selling) + " shops in " + chunks +
